...
# Calculate and visualize ARG type in each sample based on RPKM
...
# Read ARG_type file
arg_type <- read.table("D:/ARG_project/Shell/args_oap/stage_two_output/rpkm.type.txt",
                   header = TRUE, sep = "")


# Preview raw data
gather_arg_type <- gather(arg_type, key = "sample", value = "rpkm", 
...
ggplot(gather_arg_type, aes(x = sample, y = rpkm, fill = type)) + 
...
average_arg <- gather_arg_type %>%
  ...
  summarise_at(vars(rpkm), funs(mean)) # Calculate mean value of ARG type in sample type

arg_order <- unique(
  average_arg[
    order(average_arg$rpkm,
                    decreasing = T),
...
gather_arg_type$type <- factor(gather_arg_type$type, 
                               ...
                               "Others"))
## ggplot
p <- ggplot(gather_arg_type, aes(x = sample, y = rpkm, fill = type)) + 
  geom_bar(stat="identity") + 
  facet_wrap(~sample_type, scales = "free_x", labeller = label_parsed) + 
  theme_bw() + 
  xlab("") + ylab("RPKM") + 
...
# ggsave("ARG_type_rpkm.png", p, path = "D:/ARG_project/Figure/ARG",
