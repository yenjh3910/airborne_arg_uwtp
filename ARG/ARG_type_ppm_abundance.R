...
# Calculate and visualize ARG type in each sample based on PPM
...
# Read ARG_type file
arg_type <- read.table("D:/ARG_project/Shell/args_oap/ARG/stage_two_output/ppm.type.txt",
                   header = TRUE, sep = "")


# Preview raw data
gather_arg_type <- gather(arg_type, key = "sample", value = "ppm", 
...
ggplot(gather_arg_type, aes(x = sample, y = ppm, fill = type)) + 
...
average_arg <- gather_arg_type %>%
  ...
  summarise_at(vars(ppm), funs(mean)) # Calculate mean value of ARG type in sample type

arg_order <- unique(
  average_arg[
    order(average_arg$ppm,
                    decreasing = T),
...
gather_arg_type$type <- factor(gather_arg_type$type, 
                               levels = c("Multidrug", "Tetracycline",
                                          "MLS", "Sulfonamide",
                                          ...
                                          "Others"))
## ggplot
p <- ggplot(gather_arg_type, aes(x = sample, y = ppm, fill = type)) + 
  geom_bar(stat="identity") + 
  facet_wrap(~sample_type, scales = "free_x", labeller = label_parsed) + 
  theme_bw() + 
  xlab("") + ylab("PPM") + 
