# all_sample_ARG_ion_procrustes.R
library(tidyverse)
library(openxlsx)
library(vegan)
# Import & prepare input dataframe
ion <- read.xlsx("../../airborne_arg_uwtp_result/metal_ion/UWTP_ion.xlsx", sheet = 1)
row.names(ion) <- ion[,1]
ion <- ion[,-1]
ion <- as.data.frame(t(ion))
ion =decostand(ion, method = "standardize") # Normalization between water and air
ion <- as.data.frame(t(ion))
...
# PCA for ARG
arg.hel=decostand(arg_subtype, method = "hellinger")
...
p <- ggplot(Y) +
  ...
  annotate('text', label = sprintf('M^2 == 0.2600'),
           x = 0.25, y = 0.38, size = 5, parse = TRUE) +
  annotate('text', label = 'P==0.001',
           x = 0.25, y = 0.31, size = 5, parse = TRUE) +
...
# # Save
# ggsave("all_sample_ARG_ion_airwater_procrustes.png", p, path = "D:/ARG_project/Figure/procrustes",
