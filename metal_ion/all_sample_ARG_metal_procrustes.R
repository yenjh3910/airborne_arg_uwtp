# all_sample_ARG_metal_procrustes.R
library(tidyverse)
library(openxlsx)
library(vegan)
# Import & prepare input dataframe
metal <- read.xlsx("D:/ARG_project/metal_ion/UWTP_metal.xlsx", sheet = 1)
row.names(metal) <- metal[,1]
metal <- metal[,-1]
metal <- as.data.frame(t(metal))
metal =decostand(metal, method = "standardize") # Normalization between water and air
metal <- as.data.frame(t(metal))
...
p <- ggplot(Y) +
  ...
  annotate('text', label = sprintf('M^2 == 0.4632 '),
           x = 0.2, y = 0.38, size = 5, parse = TRUE) +
  annotate('text', label = 'P==0.001',
           x = 0.2, y = 0.31, size = 5, parse = TRUE) +
...
# # Save
# ggsave("all_sample_ARG_metal_airwater_procrustes.png", p, path = "D:/ARG_project/Figure/procrustes",
